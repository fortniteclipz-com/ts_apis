...
def run(event, context):
    try:
        ...
        if stream._status_initialize == ts_model.Status.DONE and stream._status_analyze == ts_model.Status.WORKING:
            stream_segments = ts_aws.rds.stream_segment.get_stream_segments(stream)
            downloaded_stream_segments = list(filter(lambda ss: ss._status_download == ts_model.Status.DONE, stream_segments))
            analyzed_stream_segments = list(filter(lambda ss: ss._status_analyze == ts_model.Status.DONE, stream_segments))
